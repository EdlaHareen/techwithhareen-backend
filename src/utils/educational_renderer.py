...
def render_educational_carousel(
    ...
    Args:
        ...
        stats:            Content items — shape depends on carousel_format:
                            ...
                            C: single-line tip per item (≤80 chars)
        image_bytes:      Optional cover image bytes.
        hook_stat_value:  Concept count for Format B Slide 2 (e.g. "5").
        hook_stat_label:  Label for Format B Slide 2 (e.g. "KEY PRINCIPLES TO MASTER").
    ...
    if carousel_format == "C":
        slides = _render_format_c(headline, stats, image_bytes, has_read_more, source_url)

    elif carousel_format == "A":
        slides = _render_format_a(headline, stats, image_bytes, has_read_more, source_url)

    elif carousel_format == "B":
        slides = _render_format_b(
            headline, stats, image_bytes,
            hook_stat_value, hook_stat_label,
            has_read_more, source_url,
        )

    else:
        # Legacy: step-by-step (no carousel_format or unknown value)
        slides = _render_legacy(headline, stats, image_bytes, has_read_more, source_url)
    ...
    logger.info(
        f"educational_renderer: {len(paths)} slides for '{headline[:50]}' (format={carousel_format})"
    )
    return paths
...
def _render_format_a(
    ...
    slides: list[Image.Image] = [
        ...
        _slide_format_a_hook(2, total),
    ]

    # Build content slides + bookmark in one pass (same pattern as carousel_renderer.py)
    ...
    # Assemble: interleave bookmark after halfway point
...
def _render_format_c(
    ...
    slides: list[Image.Image] = [
        _slide_cover(headline, image_bytes, total),
        _slide_cheat_intro(2, total),
    ]
    slides.extend(_slide_cheat_batch(b, 3 + i, total) for i, b in enumerate(batches))

    if has_read_more:
        slides.append(_slide_read_more(source_url, len(slides) + 1, total))
    slides.append(_slide_cta_save(total))
...
def _render_legacy(
    ...
    slides: list[Image.Image] = [
        _slide_cover(headline, image_bytes, total),
        _slide_learn_preview(stats, total),
    ]
    for j, chunk in enumerate(stat_chunks, start=1):
        slides.append(_slide_content(chunk, j + 1, total))
