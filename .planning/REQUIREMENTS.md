...
- [x] **EDU-01**: v2 Web UI research trigger has a content type selector — "News" / "Educational"
- [x] **EDU-02**: When "Educational" selected, ResearchOrchestrator produces exactly 1 Story with lesson steps in `key_stats`; image query targets tool visuals (logo, screenshot), not news thumbnails
- [x] **EDU-03**: Educational carousel renders a "WHAT YOU'LL LEARN" slide (Slide 2) instead of a hook stat; all other slides show numbered lesson steps
...
- [x] **EDU-06**: Caption Writer `educational` voice mode — hook teaches, body previews guide value, CTA: "DM me [KEYWORD] for the full guide 📩"
- [x] **EDU-07**: Web UI approval queue shows PDF preview (link/iframe) and DM keyword badge on educational PostCards
